...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-browser/skills/browser-intent). Original name: browser-intent. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Browser Intent

Natural-language layer on top of the low-level `browser_*` selector tools. Where `ruflo-browser-extract` and `ruflo-browser-form-fill` compose selector-based primitives (`browser_click`, `browser_fill`, `browser_snapshot`), `ruflo-browser-intent` lets the caller say what they want ("Click the login button", "Fill the search box with cats and submit") and delegates execution to [page-agent](https://github.com/alibaba/page-agent) — in-page injected JS that turns the DOM into text and drives an LLM tool-call loop against it.
...
4. **Prefer a recorded session** (`ruflo-browser-record`) when the interaction matters enough to replay later; `browser_act` itself does not open an RVF container — it operates on whatever session id you pass (or `"default"`).
