...
<!-- Incorporated from Significant-Gravitas/AutoGPT (fork khanashraf6742-boop/AutoGPT@45275cb, .claude/skills/pr-address). Original name: pr-address. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
This is an **option, not a required step**. Skip it whenever the PR already has reviews (bot or human) — those are what you are here to address. See the `autogpt-open-pr` skill's "Review workflow" step for the same trigger in the PR-creation flow.
