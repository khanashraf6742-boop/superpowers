...
<!-- Incorporated from Significant-Gravitas/AutoGPT (fork khanashraf6742-boop/AutoGPT@45275cb, .claude/skills/pr-polish). Original name: pr-polish. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# PR Polish

**Goal.** Drive a PR to merge-ready by alternating `/autogpt-pr-review` and `/autogpt-pr-address` until **all** of the following hold:

1. The most recent `/autogpt-pr-review` produces **zero new findings** (no new inline comments, no new top-level reviews with a non-empty body).
...
- `Round {current}: /autogpt-pr-review + /autogpt-pr-address on PR #{N}` — current iteration.
...
while round < _MAX_ROUNDS:
    ...
    invoke_skill("autogpt-pr-review", PR)   # posts findings as inline comments / top-level review
    findings = diff_state(PR, baseline)
    if findings.total == 0:
        break  # no new findings → go to polish polling
    invoke_skill("autogpt-pr-address", PR)  # resolves every unresolved thread + CI failure
...
`invoke_skill("autogpt-pr-review", PR)` is the agent's **own** review and always runs every round. In addition to it, the repo's review bot can be summoned on demand by commenting `/review` on the PR (see the `autogpt-open-pr` skill):

```bash
gh pr comment "${PR}" --body "/review"
```

This is an **option, not a required step**, and it is **not** a substitute for the `/autogpt-pr-review` round — use it when you want a second opinion the outer loop cannot produce itself, or when the PR carries no bot findings at all yet. Bot findings land as inline threads and top-level reviews, exactly like the agent's own findings, so both feed the same `invoke_skill("autogpt-pr-address", PR)` step — no separate handling is needed. If you do request one, the exit conditions must wait for it; see [Waiting on a requested bot review](#waiting-on-a-requested-bot-review).

### Snapshotting state

Before each `/autogpt-pr-review`, capture a baseline so the diff after the review reflects **only** what the review just added (not pre-existing threads):
...
After `/autogpt-pr-review` runs, any of these counting as "new findings" means another address round is needed:
...
If any of the four buckets is non-empty → not done; invoke `/autogpt-pr-address` and loop.

## Polish polling

Once `/autogpt-pr-review` produces zero new findings, do **not** exit yet. Bots (coderabbitai, sentry, autogpt-pr-reviewer) commonly post late reviews after CI settles — 30–90 seconds after the final push. Poll at 60-second intervals:
...
while clean_polls < required_clean:
    ...
    # must trigger /autogpt-pr-address. "success", "skipped", "neutral" are clean;
    ...
    if any ci.conclusion in NON_SUCCESS_TERMINAL:
        invoke_skill("autogpt-pr-address", PR)  # address failures + any new comments
    ...
    if threads or new_issue_comments or new_reviews:
        invoke_skill("autogpt-pr-address", PR)
...
`/autogpt-pr-address` polling inside a single round already re-checks its own comments, but `/autogpt-pr-polish` sits a level above and must also catch:
...
```python
Skill(skill="autogpt-pr-review",  args=pr_url)
Skill(skill="autogpt-pr-address", args=pr_url)
```

After each child invocation, re-query GitHub state directly — never trust a summary for the stop condition. The orchestrator's `ORCHESTRATOR:DONE` is verified against actual GraphQL / REST responses per the rules in `autogpt-pr-address`'s "Verify actual count before outputting ORCHESTRATOR:DONE" section.

### **Auto-continue: do NOT end your response between child skills**

`/autogpt-pr-polish` is a single orchestration task — one invocation drives the PR all the way to merge-ready. When a child `Skill()` call returns control to you:
...
### **Run /autogpt-pr-polish in the foreground — never in a background agent**

Spawning `/autogpt-pr-polish` inside an `Agent(subagent_type="general-purpose")` background task **does not work**. Background agents don't inherit the parent's slash-command registry, so `Skill(skill="autogpt-pr-review")` and `Skill(skill="autogpt-pr-address")` calls aren't available — the agent has to manually replicate the child skills' logic, which is fragile and tends to stall on the first network or rate-limit hiccup. Symptom: the background task reports `stalled: no progress for 600s` mid-review.

Run `/autogpt-pr-polish` inline in the foreground conversation. If the user asks for "/autogpt-pr-polish + /autogpt-pr-test in parallel", split them: foreground `/autogpt-pr-polish`, and ONLY then can the test step go to a background agent (because `/autogpt-pr-test` doesn't itself need to invoke skills).

### **You MUST invoke `Skill(autogpt-pr-review)` every round — even when bot reviews already exist**

A common failure mode: CodeRabbit / autogpt-pr-reviewer / Sentry have already posted findings on the PR, and the orchestrator skips the `Skill(autogpt-pr-review)` step on the assumption that "review has been done." That's wrong — the outer loop's purpose is to layer **the agent's own review** on top of the bot reviews, catching issues the bots miss (architecture, naming, cross-file invariants, hidden coupling). If the orchestrator only addresses bot findings without ever running its own review, the loop converges to "bot-clean" but not "agent-reviewed-clean," and the user reasonably asks "did /autogpt-pr-polish even read the diff?"

**Self-check before reporting `ORCHESTRATOR:DONE`:** confirm at least one `Skill(skill="autogpt-pr-review")` call appears in the current orchestration. If none, the loop is incomplete — go back and run one round.
...
- Fall back to REST for reads (flat `/pulls/{N}/comments`, `/pulls/{N}/reviews`, `/issues/{N}/comments`) per the `autogpt-pr-address` skill's GraphQL-fallback section.
...
- Every `/autogpt-pr-review` round checks for **duplicate** concerns first (via `autogpt-pr-review`'s own "Fetch existing review comments" step) so the loop does not re-post the same finding that a prior round already resolved.
...
- "loop /autogpt-pr-review + /autogpt-pr-address until done"
- "make sure the PR is actually merge-ready"

Do **not** use when:
- User wants just one review pass (→ `/autogpt-pr-review`).
- User wants to address already-posted comments without further self-review (→ `/autogpt-pr-address`).
