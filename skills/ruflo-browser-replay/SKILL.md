...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-browser/skills/browser-replay). Original name: browser-replay. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Browser Replay

Re-drive a recorded session trajectory. Used for regression testing, deterministic re-runs, and as the verification path that `ruflo-browser-record` plus `browser-selectors` actually produces something replayable.
...
- Comparing two runs of the same flow for `ruflo-browser-screenshot-diff`.
...
- For visual diff, chain into `ruflo-browser-screenshot-diff` against the parent session id.
