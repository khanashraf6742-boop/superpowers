---
name: ruflo-cost-burn
description: Burn-rate trend over time with optional drift-alert exit code. Bins session spend into buckets, surfaces window-over-window delta, and can exit 1 when latest bucket exceeds prior mean by a configurable %. Distinct from `ruflo-cost-trend` (benchmark drift); this tracks PRODUCTION spend trajectory.
argument-hint: "[--bucket 1d] [--lookback 14d] [--alert-on-acceleration-pct 50] [--format table|json]"
allowed-tools: Bash
---

<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-cost-tracker/skills/cost-burn). Original name: cost-burn. License: MIT. Catalog: docs/incorporated-skills/README.md -->

Burn-rate trend observability. The fourth leg of the cost-tracker forward-cost stack:

| Question | Skill |
|---|---|
| "Have we crossed a threshold?" (reactive) | `ruflo-cost-budget-check` |
| "When will we cross a threshold?" (predictive) | `ruflo-cost-projection` |
| "Could we have spent less?" (comparative) | `ruflo-cost-counterfactual` |
| "Is daily burn ACCELERATING?" (trend) | **`ruflo-cost-burn`** ← this |

## Algorithm

Implementation: [`scripts/burn.mjs`](https://github.com/ruvnet/ruflo/blob/025842bb0f860a908b286846c701e156c7f3f739/plugins/ruflo-cost-tracker/scripts/burn.mjs).
...
## Distinct from `ruflo-cost-trend`

| Skill | Data source | Question |
|---|---|---|
| `ruflo-cost-trend` | `docs/benchmarks/runs/*.json` | "Is the benchmark drifting (win rate, latency)?" |
| `ruflo-cost-burn` | `cost-tracking` namespace | "Is production spend accelerating?" |
