...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-adr/skills/adr-index). Original name: adr-index. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
3. **Verify graph integrity** (optional but recommended) via the sibling `ruflo-adr-verify` skill, which runs `scripts/verify.mjs` and exits 1 on cycles.
...
semantics. Re-running `ruflo-adr-index` refreshes changed metadata in place and does
...
`ruflo-adr-index` only ever adds/upserts. If an ADR file was deleted (or a relation line removed from a surviving file), the row it wrote stays forever — `ruflo-adr-verify` won't catch it either, since an orphan has no dangling ref and forms no cycle. Use the sibling `ruflo-adr-reindex` skill to reconcile a deletion (issue #2666).

## Cross-references

- `ruflo-adr-create` — produces the ADR files this skill consumes
- `ruflo-adr-review` — runs over `adr-patterns` for compliance checks
- `ruflo-adr-verify` (sibling skill) — runs `scripts/verify.mjs` for graph-integrity gating
- `ruflo-adr-reindex` (sibling skill) — drop-and-rebuild reconcile for a deleted ADR file (this skill can only add, never remove)
