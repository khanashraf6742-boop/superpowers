...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-browser/skills/browser-extract). Original name: browser-extract. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Browser Extract

Pull structured data out of a web page. Replaces the older `ruflo-browser-scrape` skill with three new guarantees:

1. The session is a recorded RVF container (composes `ruflo-browser-record`).
...
1. **Open a recorded session** via `ruflo-browser-record` (do not call `browser_open` directly).
...
- This skill subsumes the legacy `ruflo-browser-scrape` skill; `ruflo-browser-scrape/SKILL.md` is now a thin shim that delegates here. It will be removed in plugin v0.3.0.
