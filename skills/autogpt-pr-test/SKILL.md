...
<!-- Incorporated from Significant-Gravitas/AutoGPT (fork khanashraf6742-boop/AutoGPT@45275cb, .claude/skills/pr-test). Original name: pr-test. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
Multiple worktrees share the same host — Docker infra (postgres, redis, clamav), app ports (3000/8006/…), and the test user. Two agents running `/autogpt-pr-test` concurrently will corrupt each other's state (connection-pool exhaustion, port binds failing silently, cross-test assertions). Use the root-worktree lock file to take turns.
...
Keeping the lock held past the test run is the single most common way `/autogpt-pr-test` stalls other agents. **The app staying up is orthogonal to the lock; don't conflate them.** Sibling worktrees running their own `/autogpt-pr-test` will kill the stray processes and free the ports themselves (Step 3c/3e-native handle that) — they just need the lock file gone.

Concretely, the sequence at the end of every `/autogpt-pr-test` run (success or failure) is:
...
If a sibling agent's `/autogpt-pr-test` needs to take over, it'll do the kill+rebuild dance from Step 3c/3e-native on its own — your only job is to not hold the lock file past the end of your test.
...
**Cause:** Stuck CDP connection or Chromium process tree. Seen on macOS when a prior `/autogpt-pr-test` left a zombie Chrome for Testing.
