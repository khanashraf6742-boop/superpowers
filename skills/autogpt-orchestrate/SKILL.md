...
<!-- Incorporated from Significant-Gravitas/AutoGPT (fork khanashraf6742-boop/AutoGPT@45275cb, .claude/skills/orchestrate). Original name: orchestrate. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
```bash
SKILLS_DIR="<this skill's base directory>/scripts"   # the scripts/ folder bundled next to this SKILL.md
  ...
  "agents": [
    {
      ...
      "steps": ["autogpt-pr-address", "autogpt-pr-test"],
      "checkpoints": ["autogpt-pr-address"],
      "state": "running",
      "last_output_hash": "",
      "last_seen_at": 0,
      "spawned_at": 0,
...
## Serial /autogpt-pr-test rule

`/autogpt-pr-test` and `/autogpt-pr-test --fix` run local Docker + integration tests that use shared ports, a shared database, and shared build caches. **Running two `/autogpt-pr-test` jobs simultaneously will cause port conflicts and database corruption.**

**Rule: only one `/autogpt-pr-test` runs at a time. The orchestrator serializes them.**

You (the orchestrating Claude) own the test queue:
1. Agents do `autogpt-pr-review` and `autogpt-pr-address` in parallel — that's safe (they only push code and reply to GitHub).
2. When a PR needs local testing, add it to your mental queue — don't give agents an `autogpt-pr-test` step.
3. Run `/autogpt-pr-test https://github.com/OWNER/REPO/pull/PR_NUMBER --fix` yourself, sequentially.
...
Use an existing session. **Never create a tmux session from within Claude** — it becomes a child of Claude's process and dies when the session ends. If no session exists, tell the user to run `tmux new-session -d -s autogpt1` in their terminal first, then re-invoke `/autogpt-orchestrate`.
...
WINDOW=$(bash $SKILLS_DIR/spawn-agent.sh "$SESSION" "$WORKTREE_PATH" "$SPARE_BRANCH" "$NEW_BRANCH" "$OBJECTIVE" "$PR_NUMBER" "autogpt-pr-address" "autogpt-pr-test")
...
### 1. Run /autogpt-pr-test (required, serialized, use TodoWrite to queue)

`/autogpt-pr-test` is the only reliable confirmation that the objective is actually met. Run it yourself, not the agent.

**When multiple PRs reach `pending_evaluation` at the same time, use TodoWrite to queue them:**
```
- [ ] /autogpt-pr-test https://github.com/Significant-Gravitas/AutoGPT/pull/NNNN — <feature description>
- [ ] /autogpt-pr-test https://github.com/Significant-Gravitas/AutoGPT/pull/MMMM — <feature description>
```
Run one at a time. Check off as you go.

```
/autogpt-pr-test https://github.com/Significant-Gravitas/AutoGPT/pull/PR_NUMBER
```

**/autogpt-pr-test can be lazy** — if it gives vague output, re-run with full context:

```
/autogpt-pr-test https://github.com/OWNER/REPO/pull/PR_NUMBER
Context: This PR implements <objective from state file>. Key files: <list>.
Please verify: <specific behaviors to check>.
```

Only one `/autogpt-pr-test` at a time — they share ports and DB.

### /autogpt-pr-test result evaluation
...
2. Re-brief the agent with the specific scenario that failed and what was missing:
   ```bash
   tmux send-keys -t SESSION:WIN "PARTIAL result on /autogpt-pr-test — S5 (Apply button) never appeared. The AI must output JSON action blocks for the Apply button to render. Fix this before re-running /autogpt-pr-test."
   sleep 0.3
   tmux send-keys -t SESSION:WIN Enter
   ```
3. Set state back to `running`:
   ...
   jq --arg w "SESSION:WIN" '(.agents[] | select(.window == $w)).state = "running"' \
     ~/.claude/orchestrator-state.json > /tmp/orch.tmp && mv /tmp/orch.tmp ~/.claude/orchestrator-state.json
   ```
4. Wait for new `ORCHESTRATOR:DONE`, then re-run `/autogpt-pr-test` from scratch
...
- `/autogpt-pr-test` all scenarios PASS + evaluation looks good → mark `done` in state, tell the user the PR is ready, ask if window should be closed
- `/autogpt-pr-test` any scenario PARTIAL or FAIL → re-brief the agent with the specific failing scenario, set state back to `running` (see `/autogpt-pr-test result evaluation` above)
