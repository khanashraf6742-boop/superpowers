...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-adr/skills/adr-reindex). Original name: adr-reindex. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# ADR Reindex

`ruflo-adr-index` only ever **adds** or upserts rows — it has no way to remove one. Delete an ADR file (or a single relation line from a surviving file) and the row `ruflo-adr-index` wrote for it survives every future `ruflo-adr-index` run, forever, with no dangling-ref or cycle ever pointing at it. `ruflo-adr-verify` then certifies the resulting graph as healthy, because an orphan row with zero edges in or out is invisible to both its checks. See issue #2666.

This is a different failure mode from staleness (an ADR that changed but whose stored record didn't) — that's convergence, `ruflo-adr-index`'s job. This is **reaping** — the source of truth (the ADR file) is gone, so the derived cache row for it must be gone too. The only reliable way to reap is to drop both namespaces and rebuild from what's actually on disk right now.

## When to use

- After deleting an ADR file (or removing a relation line from one)
- Periodically, as a scheduled reconcile (`ruflo-adr-verify` can't catch what `ruflo-adr-reindex` catches — see below)
- If `ruflo-adr-verify`'s ADR count looks higher than `find docs/adr -name '*.md' | wc -l`
...
- `ADR_ROOT=/path` — scan root **and** the root the underlying `memory purge`/`memory store` subprocesses run from (must match whatever root `ruflo-adr-index` was last run with, or you'll reconcile the wrong repo's namespace)

## What it does

1. **Purge** — hard-deletes every row in `adr-patterns` and `adr-edges` via `memory purge --namespace <ns> --force` (the CLI's real `DELETE FROM memory_entries`, not `memory delete`'s soft tombstone that still blocks a same-key re-store — see "Why not `ruflo-adr-index` + `memory delete`" below).
2. **Rebuild** — re-scans every ADR currently on disk (same dual-format parser `ruflo-adr-index` uses) and stores it fresh.
3. **Post-condition** — re-lists `adr-patterns` and asserts the count equals the number of ADR files just scanned. This is stronger than `ruflo-adr-index`'s "N stored, 0 errors" tally: if a concurrent memory.db writer clobbered the purge (see Caveats), the store loop would still report success on every call — only a fresh recount catches that.

Exits non-zero if the post-condition fails.

## Why not `ruflo-adr-index` + `memory delete`
