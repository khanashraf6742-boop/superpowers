...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-cost-tracker/skills/cost-booster-route). Original name: cost-booster-route. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
Before a batch of similar tasks, or when `ruflo-cost-report` shows Sonnet/Opus spend on descriptions that look like simple transforms (`var-to-const`, `add-types`, `add-error-handling`, `async-await`, `add-logging`, `remove-console`).
...
6. **Persist** — `memory_store --namespace cost-tracking --key "booster-route-$(date +%Y%m%d-%H%M%S)" --value '{"tier1": N, "tier2_or_3": M, ...}'` so `ruflo-cost-report` picks up the tier signal.
...
ADR-0002 Decision #1 · ruflo-intelligence ADR-0001 §"Neutral" (closes the routing-outcomes loop via `ruflo-cost-optimize` step 8) · CLAUDE.md root §"3-Tier Model Routing (ADR-026)".
