...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-cost-tracker/skills/cost-track). Original name: cost-track. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Cost Track

Reads the active Claude Code session jsonl (`~/.claude/projects/<encoded-cwd>/<session>.jsonl`), tallies assistant-message `usage` per model, computes USD cost using REFERENCE.md pricing, and writes a structured record to the `cost-tracking` AgentDB namespace. This is the **producer** that gives `ruflo-cost-report` and `ruflo-cost-optimize` real data to consume.
...
3. **Verify persistence** — `mcp__plugin_ruflo-core_ruflo__memory_search --query "session-" --namespace cost-tracking` should list the new record. `ruflo-cost-report` step 1 reads from this namespace.
...
- `ruflo-cost-report` (consumer) — reads records produced by this skill
- REFERENCE.md "Cost attribution formula" — the math the script implements
- `ruflo-cost-budget-check` (consumer; landing in P2) — reads totals to evaluate alerts
