...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-cost-tracker/skills/cost-benchmark). Original name: cost-benchmark. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Cost Benchmark

Runs `scripts/bench.mjs` against the structural+adversarial corpus and writes per-case + summary results to `docs/benchmarks/runs/`. This is the verification gate that backs every measurable claim in `ruflo-cost-booster-edit` / `ruflo-cost-booster-route`.
...
4. **Read it back** in subsequent skills (e.g. `ruflo-cost-report` step 2 reads `latest.json` for live tier-spend numbers).
...
ADR-0002 §"Decision 1" / §"Riskiest assumption" · `ruflo-cost-booster-edit/SKILL.md` (verification table consumes this skill's output) · `ruflo-cost-report/SKILL.md` step 2 (reads `runs/latest.json`).
