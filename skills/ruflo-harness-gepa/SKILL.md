---
name: ruflo-harness-gepa
description: "Inspect and audit GEPA genomes via the `@metaharness/darwin/gepa` library entry (darwin 0.8.0) — load/validate a genome (default is the shipped cand-6 promotion), render the system prompt a genome compiles to, or classify failure modes in a run transcript. The `gepaOptimize` loop itself is library-only (bring your own evaluator) and not surfaced here — use `ruflo-harness-evolve` for sandbox-scored evolution. Degrades gracefully when @metaharness/darwin is absent."
argument-hint: "--op genome|validate|render|analyze [--path <genome.json>] [--transcript <t.json>] [--alert-on-invalid]"
allowed-tools: Bash
---

<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-metaharness/skills/harness-gepa). Original name: harness-gepa. License: MIT. Catalog: docs/incorporated-skills/README.md -->
...
2. **Sandbox-scored evolution**: `ruflo-harness-evolve` (darwin CLI `evolve`),
   which pairs GEPA with its own sandbox evaluators.

## Algorithm

Implementation: [`scripts/gepa.mjs`](https://github.com/ruvnet/ruflo/blob/025842bb0f860a908b286846c701e156c7f3f739/plugins/ruflo-metaharness/scripts/gepa.mjs).
