...
<!-- Incorporated from ruvnet/ruflo (fork khanashraf6742-boop/ruflo@025842b, plugins/ruflo-cost-tracker/skills/cost-budget-check). Original name: cost-budget-check. License: MIT. Catalog: docs/incorporated-skills/README.md -->

# Cost Budget Check

Reads `cost-tracking:budget-config` for the project's budget limit, sums `total_cost_usd` across `session-*` records produced by `ruflo-cost-track`, computes utilization, and emits the **measured** 4-tier alert ladder (50% INFO / 75% WARNING / 90% CRITICAL / 100% HARD_STOP).
...
- Before spawning a swarm — if utilization ≥ 90%, escalate to `/ruflo-cost-optimize` first.
...
| 75% | WARNING 🟠 | display warning, suggest `/ruflo-cost-optimize` |
| 90% | CRITICAL 🔴 | urgent alert, recommend model downgrades |
| 100% | HARD_STOP 🛑 | halt non-essential spawns; exit code 1 |

## Cross-references

- `ruflo-cost-track` (producer) — populates `cost-tracking:session-*`
- `ruflo-cost-report` — same data source, narrative format
- `ruflo-cost-optimize` — recommended action when WARNING/CRITICAL
